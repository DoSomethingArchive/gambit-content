...
function customMsgDecorator(fn) {
  return (args) => {
    fn(args);
    return helpers.sendResponse(args.res, 200, args.text);
  };
}

function renderEndConversationMsg(fn) {
  return (type, args) => helpers.renderMessageForMessageType(args.req, type)
      .then((replyMessage) => {
        args.text = replyMessage;
        fn(args);
        args.req.user.postDashbotOutgoing(args.text);
        stathat.postStat(`campaignbot:${args.text}`);
        BotRequest.log(args.req, 'campaignbot', type, args.text);
        return helpers.sendResponse(args.res, 200, args.text);
      })
...
module.exports.sendCustomMessage = function sendCustomMessage(args) {
  return new Command(
    customMsgDecorator(actions.endConversation),
    [args]);
};
