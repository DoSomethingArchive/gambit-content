 ...
 * @param {Object} data - payload to be sent in the POST request to Rogue
 */
function executeV3PostsPost(endpoint, data) {
  const fileProperty = rogueClientConfig.photoPostCreation.fileProperty;
  const client = rogueClient.request('clientCredentials').post(getRogueUrl(endpoint, 'v3'));
  /**
   * We need to setup the superagent request differently for V3 POST /posts requests.
   * The request has to be of Content-Type: multipart/form-data, but it must not be set manually.
   * It detects it by using the .field and .attach API methods.
   */
  Object.keys(data).forEach((key) => {
    if (key === fileProperty) {
      client.attach(fileProperty, data[key], 'post-photo');
    } else {
      client.field(key, data[key]);
    }
  });
  /**
   ...
   */
  client.field('action', 'default');
  return client.then(res => res.body);
}
/**
 * createPost
 *
...
module.exports.createPost = function (data) {
  // TODO: remove conditional when we fully switch to Rogue V3
  if (rogueClientConfig.useV3()) {
    return executeV3PostsPost('posts', data, 'v3', 'clientCredentials');
  }
...
module.exports.createSignup = function (data) {
  // TODO: remove conditional when we fully switch to Rogue V3
  if (rogueClientConfig.useV3()) {
    return executePost('signups', data, 'v3', 'clientCredentials');
  }
  // TODO: Remove when we fully switch to Rogue V3
