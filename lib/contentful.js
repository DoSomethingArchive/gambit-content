...
/**
 * Returns Contentful entries that have a response reference field set to given contentfulId.
 * @param {String} contentfulId
 * @return {Promise}
 */
...
function fetchEntriesWithContentTypes(contentTypes) {
  const query = module.exports.getQueryBuilder()
    ...
    .build();
  logger.debug('fetchEntriesWithContentTypes', { query });
  return module.exports.getClient().getEntries(query)
    .then((res) => {
      logger.debug('fetchEntriesWithContentTypes count', res.total);
...
/**
 * Returns Contentful entries that have a campaign reference field set to given contentfulId.
 * @param {String} contentfulId
 * @return {Promise}
 */
function fetchByContentTypeAndCampaignReferenceContentfulId(contentType, contentfulId) {
  const query = module.exports.getQueryBuilder()
    .custom({
      // Note: Contentful doesn't allow an IN operator here, we need to run separate queries
      // for each content type that can be used as a topic having a campaign.
      // @see https://www.contentful.com/developers/docs/concepts/links/#fetching-resources-linked-to-a-specific-entry
      content_type: contentType,
      'fields.campaign.sys.id': contentfulId,
    })
    .build();
  logger.debug('fetchByCampaignReferenceContentfulId', { query });
  return module.exports.getClient().getEntries(query)
    .then((res) => {
      logger.debug('fetchByCampaignReferenceContentfulId count', res.total);
      return Promise.resolve(res.items);
    })
    .catch(error => Promise.reject(module.exports.contentfulError(error)));
}
...
module.exports = {
  ...
  fetchByContentTypeAndCampaignReferenceContentfulId,
  fetchByContentfulId,
