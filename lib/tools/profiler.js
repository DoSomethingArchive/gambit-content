...
class Profiler extends EventEmitter {
  ...
  constructor(options) {
    super();
    this.extension = Profiler.sanitize(options.extension || '');
    this.tag = Profiler.sanitize(options.tag || '');
    this.interval = parseInt(options.interval, 10);
    this.namePrefix = Profiler.sanitize(options.namePrefix || '');
  ...
  */
  getName(timestamp) {
    const time = timestamp || Profiler.getTimestamp();
    return `${this.namePrefix}_${this.tag}_${time}.${this.extension}`;
  }
