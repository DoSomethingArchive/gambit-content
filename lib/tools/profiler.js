...
class Profiler extends EventEmitter {
  ...
  constructor(options) {
    super();
    this.extension = this._sanitize(options.extension) || '';
    this.tag = this._sanitize(options.tag) || '';
    this.interval = parseInt(options.interval, 10) || DEFAULT_INTERVAL_MS;
    this.namePrefix = this._sanitize(options.namePrefix) || NAME_PREFIX;
  ...
  */
  _getName(timestamp) {
    const _timestamp = timestamp || this._getTimestamp();
