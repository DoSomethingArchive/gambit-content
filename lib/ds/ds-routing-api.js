...
var tips_api = require('./tips-api');

var MCRouting = function(app) {
  this.app = app;
  this.tips_api = new tips_api(app);
...
MCRouting.prototype.handleStartCampaignResponse = function(request, response) {
  ...
  var csConfig = this.campaign_start_config[optinPathId];
  ...
  if (firstWord === '1' || firstWord === 'KNOW' ) {
    this.tips_api.deliverTips(request, response, csConfig.KNOW);
  }
  else if (firstWord === '2' || firstWord === 'PLAN' ) {
    this.tips_api.deliverTips(request, response, csConfig.PLAN);
  }
  else if (firstWord === '3' || firstWord === 'DO' ) {
    this.tips_api.deliverTips(request, response, csConfig.DO);
  }
