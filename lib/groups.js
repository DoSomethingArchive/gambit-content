  ...
  findOrCreateGroup(campaignId, campaignRunId) {
    return new Promise((resolve, reject) => {
      this.findGroup(campaignId, campaignRunId)
        .then((groups) => {
          logger.info(`Messaging Groups found, campaign ${campaignId} run ${campaignRunId}: ${groups.doing} / ${groups.completed}`);
          resolve(groups);
        })
        .catch(() => {
          // Groups don't exist yet, let's create them then.
          this.createGroup(campaignId, campaignRunId)
            .then((groups) => {
              logger.info(`Messaging Groups created, campaign ${campaignId} run ${campaignRunId}: ${groups.doing} / ${groups.completed}`);
              resolve(groups);
            })
